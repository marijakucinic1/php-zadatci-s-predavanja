<?php
// vjezba13/news.php — Inicijalizacija sjednice (session)
session_start();
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($ime === "" || $email === "" || !in_array($kategorija, $dozvoljenKat)) {
    ...
    $_SESSION["korisnik_email"]      = $email;
    $_SESSION["korisnik_kategorija"] = $kategorija;
    $poruka = "Sjednica je pokrenuta! <a href='index.php'>Idi na početnu stranicu →</a>";
  }
}

// Dohvati trenutne vrijednosti (iz POST ili iz sessiona)
$trenutnoIme   = isset($_POST["ime"])        ? $_POST["ime"]        : (isset($_SESSION["korisnik_ime"])        ? $_SESSION["korisnik_ime"]        : "");
$trenutnoEmail = isset($_POST["email"])      ? $_POST["email"]      : (isset($_SESSION["korisnik_email"])      ? $_SESSION["korisnik_email"]      : "");
$trenutnoKat   = isset($_POST["kategorija"]) ? $_POST["kategorija"] : (isset($_SESSION["korisnik_kategorija"]) ? $_SESSION["korisnik_kategorija"] : "");
?>
<!DOCTYPE html>
<html lang="hr">
<head>
  ...
  <title>Pretplata na novosti — Sjednica</title>
  <style>
...
<body>
  <main class="wrap">
    ...
    <p class="opis">Unesite podatke za personaliziranu pretplatu. Podaci se čuvaju u sjednici (trajaju dok je preglednik otvoren).</p>

    <form method="post" action="news.php">
      <label for="ime">Vaše ime</label>
      <input type="text" id="ime" name="ime"
             value="<?php echo h($trenutnoIme); ?>"
             placeholder="npr. Ana Anić" required>

      <label for="email">E-mail adresa</label>
      <input type="email" id="email" name="email"
             value="<?php echo h($trenutnoEmail); ?>"
      ...
      <select id="kategorija" name="kategorija">
        ...
        ?>
          <option value="<?php echo h($val); ?>" <?php echo $trenutnoKat === $val ? "selected" : ""; ?>>
            <?php echo h($label); ?>
          </option>
        <?php endforeach; ?>
      </select>

      <button class="btn" type="submit">Pokreni sjednicu</button>
    </form>
...
<!-- Naziv datoteke: vjezba13/news.php -->
